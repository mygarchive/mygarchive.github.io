  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden pb-12" dir="rtl">
      ...
      <div className="relative z-10 max-w-5xl mx-auto p-4 md:p-8">
          ...
          <div className="lg:col-span-3 flex flex-col justify-between">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 bg-slate-950/60 p-5 rounded-2xl border border-slate-900 text-sm">
                ...
                <p className="text-slate-400">💻 توسعه‌دهنده: <span className="text-teal-400 font-bold">{game.developers?.map((d: any) => d.name).join(' Pall ') || 'ثبت نشده'}</span></p>
        ...
        {/* 📝 توضیحات و داستان بازی استخراج شده مستقیم از بک‌آند */}
        {game.description_raw && (
          <section className="mb-8 bg-slate-900/30 border border-slate-900 p-6 rounded-3xl backdrop-blur-sm">
            <h3 className="text-lg font-bold mb-4 text-slate-300 border-r-4 border-purple-500 pr-2">📝 درباره بازی</h3>
            <p className="text-slate-300 text-sm md:text-base leading-relaxed text-justify whitespace-pre-line font-medium" dir="ltr">
              {game.description_raw}
            </p>
          </section>
        )}
