...
// ⚙️ آیدی‌های شبکه اجتماعی پشتیبانی (در صورت نیاز تغییر دهید)
const TELEGRAM_USERNAME = "YourTelegramID"; // آیدی تلگرام شما (بدون @)
const BALE_USERNAME = "YourBaleID";         // آیدی بله شما (بدون @)
...
export default function CatalogPDF() {
  // مدیریت استیت بازی‌های انتخاب‌شده در سبد
  ...
  // تولید متن استاندارد سفارش برای تلگرام، بله و کپی
  const generateOrderText = () => {
    let text = `🛒 *لیست سفارش دیتای بازی*\n\n`;
    selectedGames.forEach((g, idx) => {
      text += `${idx + 1}. ${g.name} (${g.size_gb ? g.size_gb + ' GB' : 'حجم نامشخص'})\n`;
    });
    text += `\n-------------------------\n`;
  ...
  return (
    <div className="min-h-screen bg-white text-black p-4 sm:p-6 pb-28" dir="rtl">
      {/* استایل پرینت برای خروجی PDF بی‌نقص */}
      ...
      {/* 🔴 هدر کاتالوگ (هم در سایت و هم در PDF نشان داده می‌شود) */}
      <div className="text-center mb-6 pb-4 border-b-2 border-gray-800 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="text-right">
          ...
          <p className="text-xs sm:text-sm font-bold text-gray-600">
            برای سفارش آنلاین، بازی‌ها را به سبد اضافه کرده یا نام آن‌ها را ارسال کنید.
          </p>
        </div>

        {/* آمار زنده دیتابیس + اطلاعات تماس */}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {sortedGames.map((game: any, index: number) => {
          ...
          return (
            ...
            >
              {/* نشانگر انتخاب روی کارت (در سایت) */}
              {isSelected && (
              ...
              <div className="mt-auto w-full border-t border-gray-200 pt-2 flex flex-col gap-2">
                ...
                {/* 🛒 دکمه انتخاب برای سبد سفارش (مخصوص سایت - در PDF چاپ نمیشود) */}
                <button
      ...
      {/* 🛒 نوار شناور سبد سفارش در پایین صفحه (مخصوص سایت - در PDF چاپ نمیشود) */}
      ...
      {/* 📋 پاپ‌آپ پیش‌نمایش و تایید نهایی سفارش (مخصوص سایت - در PDF چاپ نمیشود) */}
      {isModalOpen && (
          ...
          <div className="bg-white text-slate-900 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95">
            ...
            <div className="flex-1 overflow-y-auto mb-4 space-y-2 pr-1">
              {selectedGames.map((g, i) => (
                <div key={g.id || i} className="flex items-center justify-between bg-slate-50 p-2.5 rounded-xl border border-slate-200">
                  <span className="text-xs font-bold text-slate-800 truncate max-w-[240px]" dir="ltr">
                    {i + 1}. {g.name}
                  </span>
            ...
            <div className="flex flex-col gap-2">
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="py-2.5 px-4 bg-sky-500 hover:bg-sky-600 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1"
                >
                  ✈️ ارسال در تلگرام
                </button>
                <button
                  onClick={handleSendBale}
                  className="py-2.5 px-4 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1"
                >
                  🟢 ارسال در بله
                </button>
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 px-4 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-xs font-bold transition-colors flex items-center justify-center gap-1"
              >
                {copied ? '✅ لیست کپی شد!' : '📋 کپی متن لیست سفارش'}
              </button>
            </div>
