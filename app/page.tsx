          ...
          <div className="bg-white text-slate-900 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] flex flex-col shadow-2xl animate-in zoom-in-95">
            ...
            <div className="flex flex-col gap-2">
              <div className="grid grid-cols-2 gap-2">
                <a
                  href={orderCart.length > 0 ? `https://t.me/${TELEGRAM_USERNAME}?text=${encodeURIComponent(generateOrderText())}` : '#'}
                  onClick={(e) => {
                  ...
                  target={orderCart.length > 0 ? "_blank" : "_self"}
                  rel="noopener noreferrer"
                ...
                <a
                  href={orderCart.length > 0 ? `https://ble.ir/${BALE_USERNAME}?text=${encodeURIComponent(generateOrderText())}` : '#'}
                  onClick={(e) => {
                    if (orderCart.length === 0) {
                      e.preventDefault();
                      alert('سبد خرید شما خالی است! لطفاً ابتدا حداقل یک بازی انتخاب کنید.');
